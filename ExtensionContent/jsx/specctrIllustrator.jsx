...
$.specctrAi = {
    ...
    createDimensionSpecs : function () {
        try {
            ...
            if(selectionLength == 0)
                return "Please select atleast single art item to spec!";
    ...
    propertyChecked : function (sourceItem) {
        ...
        try {
            ...
            if(selectionLength == 0)
             ...
             for (var i = 0; i < selectionLength; i++) {
                var obj = app.selection[i];
                if (!obj.visibilityVariable || !obj.note || obj.note.indexOf("source") != -1) {
                    if ((result = this.createPropertySpecsForItem(obj)) != true)
                        return result;
                }
            }
            app.redraw();   //Creates an 'undo' point.    
    ...
    createPropertySpecsForItem : function(sourceItem) {
        try {
            var isCheckBoxSel = this.propertyChecked(sourceItem);
            if(isCheckBoxSel != true)
            ...
            switch (sourceItem.typename) {
                case "TextFrame": 
                    infoText = this.getSpecsInfoForTextItem(sourceItem, pageItemBounds); 
                    newColor = this.legendColor(model.legendColorType); 
                    legendLayer = this.legendSpecLayer("Text Properties"); 
                    break;
            
                case "PathItem": 
                    infoText = this.getSpecsInfoForPathItem(sourceItem, pageItemBounds); 
                    legendLayer = this.legendSpecLayer("Object Properties");	
                    break;
            
                default: 
                    infoText = this.getSpecsInfoForGeneralItem(sourceItem, pageItemBounds);
                    legendLayer = this.legendSpecLayer("Object Properties");
            }
    ...
    getSpecsInfoForTextItem : function(pageItem, cssBounds) {
        var infoText = "";
